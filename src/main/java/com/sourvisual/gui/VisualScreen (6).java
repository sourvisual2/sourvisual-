...
public class VisualScreen extends Screen {
    ...
    private static final int LOGO_SIZE = 28;
    ...
    public void render(DrawContext ctx, int mouseX, int mouseY, float delta) {
        ...
        // Тонкая полоска сверху — только чтобы было чем таскать окно
        ...
        // Бейдж с логотипом "S"
        int logoX = sideX + (SIDE_W - LOGO_SIZE) / 2;
        int logoY = sideY + LOGO_TOP;
        RenderUtils.fillRounded(ctx, logoX, logoY, LOGO_SIZE, LOGO_SIZE, 8, chip);
        ctx.getMatrices().push();
        ctx.getMatrices().translate(logoX + LOGO_SIZE / 2f - 5, logoY + LOGO_SIZE / 2f - 8, 0);
        ctx.getMatrices().scale(1.5f, 1.5f, 1f);
        ctx.drawText(this.textRenderer, Text.literal("S"), 0, 0, accent, false);
        ctx.getMatrices().pop();

        // Заголовок "sour visual" по центру под бейджем
        int titleY = logoY + LOGO_SIZE + TITLE_GAP;
        ...
        // Список вкладок
        int tabsTop = searchY + SEARCH_H + TABS_TOP_GAP;
        Tab[] tabs = Tab.values();
        int rowH = 24;
        for (int i = 0; i < tabs.length; i++) {
            ...
            int iconX = sideX + 10;
            int iconY = rowY + 5;
            drawTabIcon(ctx, tab, iconX, iconY, textColor, side);
    ...
    private void drawTabIcon(DrawContext ctx, Tab tab, int x, int y, int color, int punchColor) {
        switch (tab) {
            ...
            case UTILITIES -> drawWandIcon(ctx, x, y, color);
    ...
    private void drawWandIcon(DrawContext ctx, int x, int y, int color) {
        for (int i = 0; i < 6; i++) {
            ctx.fill(x + i, y + 9 - i, x + i + 2, y + 11 - i, color);
        }
        ctx.fill(x + 6, y, x + 8, y + 4, color);
        ctx.fill(x + 4, y + 1, x + 10, y + 3, color);
    }
    ...
    private void drawThemes(DrawContext ctx, int x, int y, int w, int h) {
        ...
        for (int i = 0; i < themes.length; i++) {
            if (!query.isEmpty() && !themes[i].name.toLowerCase().contains(query)) {
                continue;
            }
            int col = shown % THEME_COLS;
            int row = shown / THEME_COLS;
