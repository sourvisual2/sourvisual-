...
public class VisualScreen extends Screen {
    ...
    public void render(DrawContext ctx, int mouseX, int mouseY, float delta) {
        ...
        ctx.getMatrices().translate(winX + 9, winY + 5, 0);
        ctx.getMatrices().scale(1.3f, 1.3f, 1f);
        ctx.drawText(this.textRenderer, Text.literal("S"), 0, 0, accent, false);
        ctx.getMatrices().pop();
        ctx.drawText(this.textRenderer, Text.literal("sour visual"), winX + 22, winY + 9, text, false);
        ...
        for (int i = 0; i < tabs.length; i++) {
            ...
            int iconX = sideX + 10;
            int iconY = rowY + 5;
            drawTabIcon(ctx, tab, iconX, iconY, iconColor, side);

            ctx.drawText(this.textRenderer, Text.literal(tab.label), sideX + 24, rowY + 8, textColor, false);
        }
    ...
    // ---------- Иконки вкладок (простые векторные фигуры, без текстур) ----------

    private void drawTabIcon(DrawContext ctx, Tab tab, int x, int y, int color, int punchColor) {
        switch (tab) {
            case VISUAL -> drawEyeIcon(ctx, x, y, color, punchColor);
            case UTILITIES -> drawSparkleIcon(ctx, x, y, color);
            case KEYBINDS -> drawKeyIcon(ctx, x, y, color);
            case THEME -> drawThemeIcon(ctx, x, y, color);
            case SETTINGS -> drawGearIcon(ctx, x, y, color);
        }
    }

    private void drawEyeIcon(DrawContext ctx, int x, int y, int color, int punchColor) {
        RenderUtils.fillRounded(ctx, x, y + 2, 10, 6, 3, color);
        RenderUtils.fillRounded(ctx, x + 3, y + 3, 4, 4, 2, punchColor);
    }

    private void drawSparkleIcon(DrawContext ctx, int x, int y, int color) {
        ctx.fill(x + 4, y, x + 6, y + 10, color);
        ctx.fill(x, y + 4, x + 10, y + 6, color);
    }

    private void drawKeyIcon(DrawContext ctx, int x, int y, int color) {
        RenderUtils.fillRounded(ctx, x, y + 2, 6, 6, 3, color);
        ctx.fill(x + 5, y + 4, x + 10, y + 6, color);
        ctx.fill(x + 8, y + 6, x + 10, y + 8, color);
    }

    private void drawThemeIcon(DrawContext ctx, int x, int y, int color) {
        ctx.fill(x + 3, y, x + 7, y + 2, color);
        RenderUtils.fillRounded(ctx, x, y + 2, 10, 7, 2, color);
    }

    private void drawGearIcon(DrawContext ctx, int x, int y, int color) {
        RenderUtils.fillRounded(ctx, x + 2, y + 2, 6, 6, 3, color);
        ctx.fill(x + 4, y, x + 6, y + 2, color);
        ctx.fill(x + 4, y + 8, x + 6, y + 10, color);
        ctx.fill(x, y + 4, x + 2, y + 6, color);
        ctx.fill(x + 8, y + 4, x + 10, y + 6, color);
    }
    ...
    private void drawKeybinds(DrawContext ctx, int x, int y, int w, int h) {
        ...
        int searchW = addBtnX - cx - 6;
        RenderUtils.fillRounded(ctx, cx, cy, searchW, SEARCH_H, 4, chip);
        drawMagnifierIcon(ctx, cx + 5, cy + 5, textDim);
        String searchDisplay = keybindSearch.isEmpty() && !searchFocused
                ? "Search..."
                : (searchFocused ? keybindSearch + "_" : keybindSearch);
        int searchTextColor = keybindSearch.isEmpty() && !searchFocused ? textDim : text;
        ctx.drawText(this.textRenderer, Text.literal(searchDisplay), cx + 16, cy + 5, searchTextColor, false);
        ...
        int listY = cy + SEARCH_H + 8;
        int listBottom = y + h - 4;
        ctx.enableScissor(x, listY, x + w, listBottom);

        String query = keybindSearch.trim().toLowerCase();
        int shown = 0;
    ...
    private void drawMagnifierIcon(DrawContext ctx, int x, int y, int color) {
        ...
        RenderUtils.fillRounded(ctx, x + 2, y + 2, 2, 2, 1, 0x00000000);
    ...
    public boolean charTyped(char chr, int modifiers) {
        if (currentTab == Tab.KEYBINDS) {
            if (addBindFormOpen && focusedField != 0) {
            ...
            if (!addBindFormOpen && searchFocused && keybindSearch.length() < 32) {
                keybindSearch += chr;
                return true;
            }
        }
    ...
    public boolean keyPressed(int keyCode, int scanCode, int modifiers) {
        ...
        if (currentTab == Tab.KEYBINDS && !addBindFormOpen && searchFocused) {
            if (keyCode == GLFW_KEY_BACKSPACE && !keybindSearch.isEmpty()) {
                keybindSearch = keybindSearch.substring(0, keybindSearch.length() - 1);
                return true;
            }
            if (keyCode == GLFW_KEY_ESCAPE) {
                searchFocused = false;
    ...
    private boolean handleKeybindsClick(int mouseX, int mouseY) {
        ...
        if (mouseX >= cx && mouseX <= cx + searchW && mouseY >= cy && mouseY <= cy + SEARCH_H) {
            searchFocused = true;
            return true;
        }
    ...
    public boolean shouldCloseOnEsc() {
        return currentTab != Tab.KEYBINDS || (!addBindFormOpen && !capturingKey && !searchFocused);
    }
}
