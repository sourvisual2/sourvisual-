...
public class VisualScreen extends Screen {
    ...
    private void drawKeybinds(DrawContext ctx, int x, int y, int w, int h) {
        ...
        for (int i = 0; i < SourVisualConfig.keyBinds.size(); i++) {
            ...
            int cardY = listY + i * (BIND_CARD_H + BIND_CARD_GAP);
            int cardW = w - pad * 2;
        ...
        if (SourVisualConfig.keyBinds.isEmpty()) {
            ctx.drawText(this.textRenderer, Text.literal("No binds yet — tap + to add one"), cx, listY, textDim, false);
        }
    }
    ...
    public boolean shouldCloseOnEsc() {
        return currentTab != Tab.KEYBINDS || (!addBindFormOpen && !capturingKey);
    }
}
